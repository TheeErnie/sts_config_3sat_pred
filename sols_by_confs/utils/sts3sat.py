# helper functions for working with sts and 3sat problems

# save list of lists or list of sets representation of sts system to 3-sat .cnf format
def sts_to_sat_file(sts: list[set[int]], order: int, file_path: str) -> None:
    with open(file_path, "w") as file:
        num_blocks = int(order * (order - 1) / 6)
        file.write(f"p cnf {order} {num_blocks}\n")
        for block in sts:
            file.write(" ".join(str(var) for var in block))
            file.write(" 0\n")

# write 3SAT to cnf file
def write_cnf_file(clauses: list, filename: str) -> None:
    n = max([max([abs(int(lit)) for lit in clause]) for clause in clauses])
...
# create random STS instances

"""
# THE FOLLOWING CODE WAS CREATED BY JOSH GANSCHOW and can be found at https://github.com/GanschowJosh/Steiner-Triple-Systems-Research/tree/main

This module houses all the functions for the generation of random Steiner Triple Systems
"""
import random
